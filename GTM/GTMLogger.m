...
-(void) setWriter:(id<GTMLogWriter> )writer {
    @synchronized(self) {
        [writer_ autorelease];
        if (writer == nil) writer_ = [[NSFileHandle fileHandleWithStandardOutput] retain];
        else writer_ = [writer retain];
    }
    GTMLOGGER_ASSERT(writer_ != nil);
}
...
-(void) setFormatter:(id<GTMLogFormatter> )formatter {
    @synchronized(self) {
        ...
        if (formatter == nil) formatter_ = [[GTMLogBasicFormatter alloc] init];
        else formatter_ = [formatter retain];
    }
    GTMLOGGER_ASSERT(formatter_ != nil);
}
...
-(void) setFilter:(id<GTMLogFilter> )filter {
    @synchronized(self) {
        [filter_ autorelease];
        if (filter == nil) filter_ = [[GTMLogNoFilter alloc] init];
        else filter_ = [filter retain];
    }
    GTMLOGGER_ASSERT(filter_ != nil);
}
...
-(void) logInternalFunc:(const char*)func
    ...
    NSString* msg = [formatter_ stringForFunc:fname
                                   withFormat:fmt
                                       valist:args
                                        level:level];
    if (msg && [filter_ filterAllowsMessage:msg level:level]) [writer_ logMessage:msg level:level];
}
...
-(void) logMessage:(NSString*)msg level:(GTMLoggerLevel)level {
    @synchronized(self) {
        ...
        GTM_FOREACH_OBJECT(child, self) {
            if ([child conformsToProtocol:@protocol(GTMLogWriter)]) [child logMessage:msg level:level];
        }
    }
}
...
static BOOL IsVerboseLoggingEnabled(void) {
    ...
    if (env == NULL) env = getenv([kVerboseLoggingKey UTF8String]);
    if (env && env[0]) {
